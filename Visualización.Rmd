...
Casos por edades de Mujeres,Hombres y Totales:
```{r}
mpgman2 <- edadsexo %>%
  count(edad,Casos,sexo) %>%
  glimpse()    

hchart(mpgman2, "column", hcaes(x = sexo, y = Casos, group = edad)) %>% hc_add_theme(hc_theme_darkunica())
```
...
ggplot() +
  ...
  theme_map()
```  
Añado un mapa mundial estatico: 
```{r}
ggplot(data = map) +
  borders("world", colour = "gray90", fill = "gray85") +
  theme_map() + 
